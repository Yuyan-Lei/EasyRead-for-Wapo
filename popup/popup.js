...
window.onload = () => {
  chrome.storage.sync.get(["extensionSwitchs"], function (result) {
    ...
    if (result.extensionSwitchs) {
      ...
      document.getElementById("turn-on-toggle").checked =
        currentSwitchStatus["turn-on-toggle"];
      ...
      chrome.storage.sync.set(
        { extensionSwitchs: currentSwitchStatus },
        function () {
          console.log(
            "first time onload: Value is set to " + currentSwitchStatus
          );
        }
      );
    }
...
document
  ...
  .addEventListener("change", onClickSimplify);
...
document
  ...
  .addEventListener("change", onChangeTargetLanguage);
...
document
  .getElementById("targetFont")
  .addEventListener("change", onChangeTargetFont);
